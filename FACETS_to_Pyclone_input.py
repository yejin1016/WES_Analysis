# Original : https://github.com/vanallenlab/PyCloneTSVGeneration_FACETS_or_TITAN/blob/master/generate_wgs_pyclone_input.py
# Using only FACET output and maf files to create PyClone input format
# Usage: python /path/to/FACETS_to_Pyclone_input.py "maf file" --facets_output "FACET file" "output dir" --sample_id "sample_id"
...
# Filter out only nonsynonymous mutations from maf file
...
## Unhash them when using X, Y chromosome
...
def main():
    parser = ArgumentParser(description='Generate .tsv input for PyClone from SNV maf,'
                                        ' GATK indel file, and FACET output')
